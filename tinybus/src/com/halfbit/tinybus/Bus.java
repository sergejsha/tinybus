 ...
 * <ol>
 * <li>Expose a <b>public</b> method, known as the <i>event handler</i>, which accepts a single argument of the type of event
 * desired;</li>
 * <li>Mark it with a {@link com.halfbit.tinybus.Subscribe} annotation;</li>
 ...
 *
 * <p>Events are routed to all subscribers registered for exactly this type of event. Event's class hierarchy is ignored.
 *
 * <p>When {@code post} is called, all registered subscribers for an event are run in sequence synchronously, so subscribers 
 * should be reasonably quick. If an event may trigger an extended process (such as a database load), spawn a thread or 
 * queue it for later.
 * 
 * <p>You are allowed to post a new event while already handling an event inside a subscriber method. In this case the bus 
 * will finalize dispatching of current event first and then a new event will be dispatched.
 *
 * <h2>Handler Methods</h2>
 * Event handler methods must accept only one argument: the event.
 *
 * <p>Handlers should not, in general, throw. If they do, the Bus will wrap the exception and re-throw it.
 *
 ...
 * producer. Null values coming from producers are <b>not</b> dispatched to subscribers.
 *
